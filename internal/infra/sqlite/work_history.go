...
// is set to 1 (top of the list), and all existing entries have their
// sort_order shifted down by one so the newest job always appears first.
func (s *Store) CreateWorkHistory(ctx context.Context, input domain.WorkHistoryInput) (domain.WorkHistoryEntry, error) {
	// Shift all existing entries down to make room at position 1.
	_, err := s.db.ExecContext(ctx,
		`UPDATE work_history_entry SET sort_order = sort_order + 1`,
	)
	if err != nil {
		return domain.WorkHistoryEntry{}, fmt.Errorf("work_history: shift sort_orders: %w", err)
	}

	result, err := s.db.ExecContext(ctx,
		`INSERT INTO work_history_entry
			(employer_name, job_title, summary, start_date, end_date,
			 date_granularity_start, date_granularity_end, sort_order)
		 VALUES (?, ?, ?, ?, ?, ?, ?, 1)`,
		input.EmployerName, input.JobTitle, input.Summary,
...
func (s *Store) GetWorkHistory(ctx context.Context, id int64) (domain.WorkHistoryEntry, error) {
	...
	err := s.db.QueryRowContext(ctx,
		...
		&entry.ID, &entry.EmployerName, &entry.JobTitle, &entry.Summary,
...
func (s *Store) UpdateWorkHistory(ctx context.Context, id int64, input domain.WorkHistoryInput) (domain.WorkHistoryEntry, error) {
	result, err := s.db.ExecContext(ctx,
		`UPDATE work_history_entry
		 SET employer_name = ?, job_title = ?, summary = ?, start_date = ?,
...
// for that entry and bullet type.
func (s *Store) CreateBullet(ctx context.Context, workHistoryID int64, text string, bulletType string) (domain.AchievementBullet, error) {
	...
	err = s.db.QueryRowContext(ctx,
		"SELECT MAX(sort_order) FROM achievement_bullet WHERE work_history_id = ? AND bullet_type = ?",
		workHistoryID, bulletType,
	).Scan(&maxOrder)
	if err != nil {
	...
	result, err := s.db.ExecContext(ctx,
		`INSERT INTO achievement_bullet
			(work_history_id, text, bullet_type, sort_order)
		 VALUES (?, ?, ?, ?)`,
		workHistoryID, text, bulletType, nextOrder,
	)
	if err != nil {
...
func (s *Store) getBullet(ctx context.Context, id int64) (domain.AchievementBullet, error) {
	...
	err := s.db.QueryRowContext(ctx,
		`SELECT id, work_history_id, text, bullet_type, sort_order, created_at, updated_at
		 FROM achievement_bullet WHERE id = ?`, id,
	).Scan(&b.ID, &b.WorkHistoryID, &b.Text, &b.BulletType, &b.SortOrder, &b.CreatedAt, &b.UpdatedAt)
	if err != nil {
...
func (s *Store) listBulletsForEntry(ctx context.Context, workHistoryID int64) ([]domain.AchievementBullet, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT id, work_history_id, text, bullet_type, sort_order, created_at, updated_at
		 ...
		 ORDER BY bullet_type, sort_order`, workHistoryID,
	)
	if err != nil {
	...
	bullets := make([]domain.AchievementBullet, 0)
	for rows.Next() {
		var b domain.AchievementBullet
		if err := rows.Scan(&b.ID, &b.WorkHistoryID, &b.Text, &b.BulletType, &b.SortOrder, &b.CreatedAt, &b.UpdatedAt); err != nil {
